<template>
    <div id="app" class="min-h-screen flex justify-center pt-12 flex-col">
        <span class="font-medium text-4xl mb-4">Agent List</span>
        ...
        <div class="rounded bg-white shadow-md" style="width: 1063px;">
            <div class="py-4 px-5 flex justify-between">
                ...
                <div class="flex">
                    <button class="border border-gray-400 rounded shadow py-1 px-2 flex items-center mr-2">
                        <svg class="mr-1" height="16" viewBox="0 0 16 16" width="16" xmlns="http://www.w3.org/2000/svg"
                            style="height: 12px; width: 12px;">
                    ...
                    <tbody>
                        <tr class="border-gray-300 border-b hover:bg-gray-100" v-for="(invoice, index) in filteredInvoices"
                            ...
                            <td>
                                <div class="flex">
                                    <button
                                        class="hover:border-gray-400 hover:shadow bg-blue-400 hover:bg-white border border-transparent rounded py-1 px-2 flex items-center">
                                        Edit
                                    </button>
                                    <button
                                        class="hover:border-gray-400 hover:shadow bg-red-500 ml-2 hover:bg-white border border-transparent rounded py-1 px-2 flex items-center">
                                        Delete
                                    </button>
                                </div>
            ...
            <div class="py-4 px-5 flex justify-between items-center">
                <p>
                    <span class="font-medium">{{ filteredLength }}</span>
                    <span v-if="filteredLength === 1">result</span>
                    <span v-else>results</span>
                </p>
                <div>

                    <span class="border border-gray-400 rounded shadow py-1 px-2 mr-1"
                        @click="setPage(currentPageIndex - 1)">Prev</span>
                    <span  @click="setPage(page)" class="pl-2" :class="{ 'current': currentPageIndex === page }">
                        {{ currentPageIndex +1 }}</span>
                    <span class="border border-gray-400 rounded shadow py-1 px-2"
...
export default {
    ...
    computed: {
        ...
        //     var filteredData = filterBy(this.data, this.search);
